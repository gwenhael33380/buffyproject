...
if($id_user) {
    $req = $db->prepare("
		SELECT u.id, u.first_name, u.last_name, u.pseudo, u.email, a.id id_article, a.title, a.content, c.id id_comment, c.comment_content, r.id id_role
		FROM users u
...
?>
    ...
    <h1 class="title">Formulaire de mise à jour de <?php echo sanitize_html($users[0]->first_name . ' ' . $users[0]->last_name); ?></h1>
    <main>
	...
	");
        ...
        foreach($users as $user) :
            if($id_user != $user->id) : // afficher un nouveau formulaire à chaque utilisateur
                ...
                ?>
                <div class="file_form">
                    <form action="<?php echo HOME_URL . 'requests/user_update_post.php'; ?>" method="POST">
                        <div>
