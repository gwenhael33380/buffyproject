...
$initial_role = intval($_POST['initial_role']);
$role_select = intval($_POST['role']);
    ...
    if(!isset($msg_error)) :
        ...
        if($send_request) :
            if ($empty_pass):
                ...
                if($same_email && $same_pseudo) :
                    $request =  "UPDATE users SET id_role = :id_role, first_name = :first_name, last_name = :last_name, password = :pass WHERE id = :id_user";

                elseif($same_email && !$same_pseudo) :
                    $request =  "UPDATE users SET id_role = :id_role, first_name = :first_name, last_name = :last_name, pseudo = :pseudo, password = :pass WHERE id = :id_user";

                elseif(!$same_email && $same_pseudo) :
                    $request =  "UPDATE users SET id_role = :id_role, first_name = :first_name, last_name = :last_name, password = :pass WHERE id = :id_user";

                elseif(!$same_email && !$same_pseudo) :
                    $request =  "UPDATE users SET id_role = :id_role, first_name = :first_name, last_name = :last_name, pseudo = :pseudo, email = :email, password = :pass WHERE id = :id_user";
                endif;
            endif;
            if ($id_image != $default_img_id){
                ...
                $req->bindValue(':id_user', $id_user, PDO::PARAM_INT);
                $req->bindValue(':id_role', $role_select, PDO::PARAM_INT);
